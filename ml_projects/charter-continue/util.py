...
NUMBER_OF_CHARS = MAX_CHAR - MIN_CHAR + 1
...
def message_to_one_hot(message: str) -> np.ndarray:
    return to_one_hot_vector([char_to_num(char) for char in message], NUMBER_OF_CHARS)

def format_one_hot_messages(data: np.ndarray) -> np.ndarray:
    data = data[-MAX_CHARS_IN_DATA:]
    padding = np.zeros([MAX_CHARS_IN_DATA - len(data), NUMBER_OF_CHARS]).flatten()
    return np.hstack([padding, data.flatten()])

NETWORK_INPUT_LAYER_SIZE = NUMBER_OF_CHARS * MAX_CHARS_IN_DATA
NETWORK_HIDDEN_LAYER_SIZE = 2**10
NETWORK_OUTPUT_LAYER_SIZE = NUMBER_OF_CHARS
...
network = nn.network.Network([
    ...
    nn.layers.Dense(NETWORK_HIDDEN_LAYER_SIZE, NUMBER_OF_CHARS),
