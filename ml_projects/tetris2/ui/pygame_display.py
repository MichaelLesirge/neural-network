...
from .tetris_ui import ToggleButton, TetrisRenderer, Tiles, GridContext
...
class PygameDisplay(Display):
    ...
    def __init__(self) -> None:
        ...
        self.screen = pygame.display.set_mode(
            (self.screen_width * start_grid_size.x,
             self.screen_height * start_grid_size.y),
            pygame.RESIZABLE,
        )
        ...
        self.tetromino_tiles = Tiles()
        self.tetromino_ghost_tiles = Tiles()

        self.buttons = [self.pause_button, self.mute_button]
    ...
    def update(self,
        ...
        self.tetris.set_board_shape(board.T.shape, (1, 1))

        self.tetris.set_boards([
            (board, self.tetromino_tiles),
            (ghost_tetromino_board, self.tetromino_ghost_tiles),
            (current_tetromino_board, self.tetromino_tiles.map(
                lambda tile: make_transparent(tile, current_tetromino_percent_placed)))
        ])

        self.tetris.set_held_tetromino(
            held_tetromino,
            (
                self.tetromino_tiles.map(gray_scale)
                if can_use_held_tetromino
                else self.tetromino_tiles
            ),
        )

        self.tetris.set_queued_tetromino(tetromino_queue, self.tetromino_tiles)

        self.tetris.set_info(info)

        self.tetris.draw(GridContext.create_from_smallest_rows_and_cols(
