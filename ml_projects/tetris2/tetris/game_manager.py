...
class TetrisGameManager:
    def __init__(self, board: Grid, tetromino_shape_queue: RefillingQueue[TetrominoShape]) -> None:
        self.board = board
    ...
    def _get_next_falling_tetromino(self) -> Tetromino:
        ...
        tetromino_start_position = (
            (self.board.get_width() - tetromino_shape.get_width(tetromino_start_orientation)) // 2,
            0
        )
    
        return Tetromino(tetromino_shape, tetromino_start_position, tetromino_start_orientation)
    ...
    def step(self, actions: list[Move]):
        ...
        did_collide = self.board.does_overlap(self.falling_tetromino.get_position(), self.falling_tetromino.get_grid_array())
        
        if did_collide:
            self.falling_tetromino.move(dy = -1)
            did_collide = self.board.insert(self.falling_tetromino.get_position(), self.falling_tetromino.get_grid_array())
            self.falling_tetromino = None